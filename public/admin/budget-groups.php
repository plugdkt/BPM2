...
?>
  ...
  <div class="card">
    ...
    <?php foreach ($rows as $r): $fid = 'grp-form-' . (int) $r['id']; $tfid = 'grp-toggle-' . (int) $r['id']; ?>
    ...
    <?php endforeach; ?>
    <form id="grp-form-new" method="post" action="<?= htmlspecialchars(bpm_url('actions/save-lookup.php'), ENT_QUOTES) ?>">
      <?= bpm_csrf_field() ?>
      <input type="hidden" name="table" value="budget_groups">
    ...
    <table class="data-table">
      ...
      <tbody>
        <?php foreach ($activeRows as $r): $fid = 'grp-form-' . (int) $r['id']; $tfid = 'grp-toggle-' . (int) $r['id']; ?>
          <tr>
        ...
        <tr>
          ...
          <td style="width:110px;"><button type="submit" form="grp-form-new" class="btn btn-primary" style="padding:6px 12px;"><?= bpm_icon('plus', 14) ?></button></td>
        </tr>
      </tbody>
    </table>
    ...
    <?php if (!empty($inactiveRows)): ?>
      <details style="margin-top:18px;">
        ...
        <table class="data-table" style="margin-top:10px;">
          <tbody>
            <?php foreach ($inactiveRows as $r): $tfid = 'grp-toggle-' . (int) $r['id']; ?>
              <tr>
                <td class="text-muted"><?= htmlspecialchars($r['name'], ENT_QUOTES) ?></td>
                <td class="text-muted" style="width:160px;"><?= htmlspecialchars($r['code'], ENT_QUOTES) ?></td>
    ...
    <p class="text-muted small" style="margin-top:14px;">ไม่มีปุ่มลบข้อมูลถาวรโดยเจตนา — ไอคอน <?= bpm_icon('trash', 12) ?> "ปิดการใช้งาน" จะซ่อนรายการออกจากทุกหน้าแต่เก็บประวัติไว้ครบ กู้คืนได้เสมอที่ "รายการที่ปิดใช้งานแล้ว" ด้านล่าง</p>
  </div>
